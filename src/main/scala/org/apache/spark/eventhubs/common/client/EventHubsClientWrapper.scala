...
private[spark] class EventHubsClientWrapper(private val ehParams: Map[String, String])
  ...
  private val connectionString =
    Try {
      if(ehNamespace.contains('.'))
        Success(new ConnectionStringBuilder(new URI("amqps://" + ehNamespace), ehName, ehPolicyName, ehPolicy).toString)
      else
        Success(new ConnectionStringBuilder(ehNamespace, ehName, ehPolicyName, ehPolicy).toString)
    } getOrElse Try {
